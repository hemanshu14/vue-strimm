<template>
  <div class="ShowsSlider" :class="showsSliderClasses">
    <div ref="list" class="Slider__list">
      <div
        ref="track"
        class="ShowsSlider__track"
        :style="{transform: `translate(${translateX + marginX}px)`, transition: `transform ${settings.timing} ${transitionDelay}ms`}"
        @mouseover="handleMouseOver('track')"
        @mouseout="handleMouseOut('track')"
      >
        <div
          :class="['ShowsSlider__slides', 'ShowsSlider__slides--cloned', settings.extraClass]"
          ref="slidesClonedBefore"
          v-if="clonedSlides"
        >
          <slot />
        </div>
        <div :class="['ShowsSlider__slides', 'ShowsSlider__slides--regular', settings.extraClass]" ref="slides">
          <slot />
        </div>
        <div
          :class="['ShowsSlider__slides', 'ShowsSlider__slides--cloned', settings.extraClass]"
    ...
    <div class="ShowsSlider__actions" v-if="!settings.disabled && (settings.navButtons || settings.dots)">
      <button
        type="button"
        ref="prevButton"
        v-if="settings.navButtons && !settings.disabled"
        class="ShowsSlider__nav-button ShowsSlider__nav-button--prev"
      ...
      <ul ref="dots" v-if="settings.dots && !settings.disabled" class="ShowsSlider__dots">
        <li
          v-for="n in slidesCount"
          :key="n" class="ShowsSlider__dot"
          :class="{'ShowsSlider__dot--current': n - 1 === currentSlide}"
      ...
      <button
        ...
        ref="nextButton"
        v-if="settings.navButtons && !settings.disabled"
        class="ShowsSlider__nav-button ShowsSlider__nav-button--next"
